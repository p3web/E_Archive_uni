...
function GetTmeStamp(inputId) {
    ...
    Date = Date.split('/');
    Date = JalaliDate.jalaliToGregorian(Date[0], Date[1], Date[2]);
    Date = Date.join('-');
    Date += ' 00:00:00';
    return Date;
}
...
function convertTimeStampToJalali(Date) {
    Date = Date.split(' ');
    Date = Date [0];
    Date = Date.split('-');
    Date = JalaliDate.gregorianToJalali(Date[0], Date[1], Date[2]);
...
function GetTimeNow() {
    ...
    var jalaliDate = JalaliDate.gregorianToJalali(year, month, Day);
    if (jalaliDate[1].toString().length < 2) {
        jalaliDate[1] = '0' + jalaliDate[1];
    }
    if (jalaliDate[2].toString().length < 2) {
